...
def handle_form():
    if request.method == 'POST':
        ...
        if user_password != confirmation_password:
            ...
            return f"Password must be more than 8 symbols {render_template('form.html')}"
        with connection:
            with connection.cursor() as cursor:
                try:
                    ...
                    return f"Thank you for registration! {render_template('login.html')}"
...
def upload_pdf():
    ...
    if request.method == 'POST':
        ...
        return f'<a href="/download_pdf_mp3/{os.path.basename(my_audio)}">Download MP3</a>'
    else:
...
def download_pdf_mp3(filename):
    return send_from_directory(app.config['UPLOAD_FOLDER_PDF'], filename, as_attachment=True)
...
def bonus_mp3():
    ...
    if request.method == 'POST':
        ...
        return f'<a href="/download_new_txt/{os.path.basename(my_text)}">Download text</a>'
    else:
